...
const {userData,setUserData, token, backendUrl, loadUserProfileData} = useContext(AppContext)
  ...
  return userData && (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      {
        isEdit ?
          <input className='bd-grey-50 text-3xl font-medium max-w-60 mt-4' type="text" value={userData.name} onChange={(e) => { setUserData(prev => ({ ...prev, name: e.target.value })) }} />
          :
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
          ...
          <p className='font-medium'>Address: </p>
          {
            isEdit ?
              <p>
                <input className='bg-gray-50' onChange={(e) => { setUserData(pre => ({ ...pre, address: { ...pre.address, line1: e.target.value } })) }} type="text" value={userData.address.line1} />
                <br />
