...
const addDoctor = async (req, res)=>{
    try{
        ...
        if(!validator.isEmail(email)){
            return res.josn({success:false, message:"Valid Email"})
        }

        //Check pass stron or not

        if(password.length < 8){
            return res.josn({success:false, message:"Enter a Strong password"})
        }


        // hassing  / encrypt password
        ...
        res.json({success:true, message:"Doctor stored"})
